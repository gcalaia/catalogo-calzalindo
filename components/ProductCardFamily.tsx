...
interface Talle { talla: string; stock: number; codigo: number; }
interface VarianteColor { color: string; imagen_codigo: number; talles: Talle[]; }
...
export default function ProductCardFamily({
  nombre, marca_descripcion, precio_lista, fecha_compra, variantes,
}: Props) {
  ...
  const stockTotal = tallesDisponibles.reduce((s, t) => s + t.stock, 0);
  ...
  const handleWhatsApp = () => {
    ...
    window.open(`https://wa.me/5491234567890?text=${encodeURIComponent(msg)}`, '_blank');
  };
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow">
      <div className="relative h-64 bg-gray-100">
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-2">
          {esNuevo && <span className="bg-green-500 text-white text-xs font-bold px-2 py-1 rounded">¡Nuevo!</span>}
          {ultimasUnidades && <span className="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded">¡Últimas unidades!</span>}
        </div>
      </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {variantes.map((v) => (
              <button
                key={v.color}
                onClick={() => { setColorSeleccionado(v.color); setTalleSeleccionado(''); setImageError(false); }}
                className={`px-3 py-2 rounded-md text-sm font-medium transition-all ${
                  colorSeleccionado === v.color ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                {v.color}
              </button>
        ...
        <div className="mb-4">
          ...
          >
            ...
            {tallesDisponibles.map((t) => (
              <option key={t.codigo} value={t.talla}>
                Talle {t.talla} - Stock: {t.stock}
              </option>
            ))}
          </select>
        </div>

        {/* Precios comerciales (.999 en todos) */}
