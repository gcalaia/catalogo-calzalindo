...
export default function ProductCard({ familia, onImageError }: ProductCardProps) {
  ...
  useEffect(() => {
    const loadImageUrl = () => {
      ...
      // Construir path de imagen basado en familia_id y codigo
      const familiaIdPadded = String(familia.familia_id || '0000000').padStart(7, '0');
      const codigoPadded = String(variante.codigo).padStart(13, '0');
      const imagePath = `imagenes_macroges/0001AR${familiaIdPadded}-${codigoPadded}000001.jpg`;
      
      // Usar el endpoint /api/img con el path
      const imageUrl = `/api/img?p=${encodeURIComponent(imagePath)}`;
      
      setImagenesUrls(prev => new Map(prev).set(selectedColor, imageUrl));
      setIsLoadingImage(false);
    };
    
    loadImageUrl();
  }, [selectedColor, familia.variantes, familia.familia_id, imagenesUrls, variantesConError]);
  ...
  return (
    <>
      <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-300 border border-gray-100">
        <div
          className="relative aspect-square bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden cursor-pointer group"
          onClick={() => setShowProductModal(true)}
        >
          {/* Badges superiores */}
          <div className="absolute top-3 left-3 right-3 flex flex-wrap gap-2 z-10">
            {esUltimasUnidades && (
              <div className="bg-orange-500 text-white text-xs font-bold px-3 py-1.5 rounded-md shadow-lg animate-pulse">
                ¡Últimas unidades!
              </div>
            )}
            {familia.destacado && (
              <div className="bg-yellow-500 text-white text-xs font-bold px-3 py-1.5 rounded-md shadow-lg">
                ⭐ Destacado
              </div>
            )}
          </div>

          {/* Indicador de "Ver detalles" al hover */}
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 flex items-center justify-center">
            <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/90 text-gray-900 px-4 py-2 rounded-lg font-semibold text-sm shadow-lg">
              👁️ Ver detalles
            </div>
          </div>

          {/* Badges inferiores: Taxonomía y Línea */}
          <div className="absolute bottom-3 left-3 right-3 flex flex-wrap gap-2 z-10">
            {familia.taxonomia && (
              <div className={`text-xs font-semibold px-3 py-1.5 rounded-md border backdrop-blur-sm ${getTaxonomyColor(familia.taxonomia)}`}>
                {getTaxonomyIcon(familia.taxonomia)} {familia.taxonomia}
              </div>
            )}
            {familia.linea && (
              <div className={`text-xs font-semibold px-3 py-1.5 rounded-md border backdrop-blur-sm ${getLineaColor(familia.linea)}`}>
                {getLineaIcon(familia.linea)} {familia.linea}
              </div>
            )}
          </div>
